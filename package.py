...
class Package(Command):
    ...
    def localize_requirements(self):
        ...
        print("local packages in wheel: {:s}".format(local_dependencies))
    ...
    def execute(self, command, capture_output=False):
        ...
        print("Running shell command: {:s}".format(command))
        ...
        if return_code != 0:
            print("Error running command {:s} - exit code: {:s}".format(command, return_code))
    ...
    def run(self):
        ...
        commands.extend([
            ...
            "mkdir {dir}".format(dir=WHEELHOUSE),
